...
export default function LoginPage() {
  const [step1, mobileAction, mobilePending] = useActionState(startLogin, initialState);
  const [step2, otpAction, otpPending] = useActionState(confirmLogin, initialState);
    ...
    <div className="flex flex-1 items-center justify-center p-4">
      <div className="glass-card w-full max-w-sm p-6">
        <div className="text-center mb-6">
          <div className="text-2xl font-bold text-[var(--ats-ink)]">ATS Connect</div>
          <div className="text-sm text-[var(--ats-muted)]">Admin Login</div>
        </div>

        {!showOtp ? (
          <form action={mobileAction} className="flex flex-col gap-3">
            <label className="text-sm font-semibold text-[var(--ats-ink)]">Mobile Number</label>
            <input
              name="mobile"
              type="tel"
              inputMode="numeric"
          ...
          <form action={otpAction} className="flex flex-col gap-3">
            ...
            <label className="text-sm font-semibold text-[var(--ats-ink)]">
              Enter OTP sent to {step1.mobile}
            </label>
            <input
              name="code"
              type="tel"
